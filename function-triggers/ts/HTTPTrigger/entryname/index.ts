...
import { teamsfx, teamsfxConfig, UserInfo } from 'teamsdev-server';
 ...
 */
export default async function run(context: Context, req: HttpRequest, config: teamsfxConfig): Promise<{ [key: string]: any; }> {
  ...
  let client: teamsfx | undefined = undefined;
  try {
    client = teamsfx.getInstance(config);
  } catch(e) {
    context.log.error(e);
    res.status = 500;
